...
export function getStepLabel(nodeId: string): string {
  return ORCHESTRATOR_STEPS.find((s) => s.nodeId === nodeId)?.label ?? nodeId;
}

export function getOrderedSteps(): StepMapping[] {
  return [...ORCHESTRATOR_STEPS].sort((a, b) => a.order - b.order);
}
...
> = {
  ...
  propose_todo_items: {
    started: "Preparing task proposal...",
    completed: "Task proposal ready",
    failed: "Could not prepare task proposal",
  },
  apply_approved_todo_items: {
    started: "Saving approved tasks...",
    completed: "Tasks saved",
    failed: "Could not save approved tasks",
  },
  vault_search: {
