...
export class InsightGenerator {
  ...
  async generateInsights(
    ...
    const { granularity, periodStart, periodEnd, provider, forceRegenerate } =
      options;
    ...
    const atoms = await this.fetchAtomsForPeriod(
      userId,
      periodStart,
      periodEnd,
      provider,
    ...
    const { summary, keyFacts, actionableInsights } =
      this.generateSummaryFromAnalysis(
        analysis,
        granularity,
        periodStart,
        periodEnd,
  ...
  async generateWeeklyPattern(
    ...
    const { summary, keyFacts, actionableInsights } =
      this.generateWeeklySummary(analysis, patterns, periodStart, periodEnd);
  ...
  private analyzeAtoms(atoms: StoredActivityAtom[]): CalendarInsight {
    ...
    const meetings = events.filter(
      (a) => a.participants && a.participants.length > 0,
    );
    ...
    const averageEventsPerDay =
      uniqueDays > 0 ? Math.round((events.length / uniqueDays) * 10) / 10 : 0;
  ...
  private detectPatterns(
    ...
    const meetingRatio =
      events.length > 0 ? meetingEvents.length / events.length : 0;
    ...
    for (let i = 1; i < sortedEvents.length; i++) {
      ...
      const prevEnd =
        prev.occurredAt.getTime() + (prev.durationMinutes || 0) * 60000;
  ...
  private generateSummaryFromAnalysis(
    ...
    summaryParts.push(
      `${analysis.totalEvents} total events, including ${analysis.totalMeetings} meetings.`,
    );
    ...
    keyFacts.push(
      `${Math.round(analysis.totalMeetingMinutes / 60)} hours in meetings`,
    );
    ...
    if (
      analysis.patterns.includes(
        "Meeting-heavy schedule with limited focus time",
      )
    ) {
    ...
    if (
      analysis.patterns.includes(
        "Frequent back-to-back meetings with little buffer time",
      )
    ) {
  ...
  private generateWeeklySummary(
    ...
    const base = this.generateSummaryFromAnalysis(
      analysis,
      "week",
      periodStart,
      periodEnd,
